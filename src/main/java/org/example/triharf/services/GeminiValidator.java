...
public class GeminiValidator {
    ...
    public ValidationResponse validateWord(String word, String categoryName, Character letter) {
        if (API_KEY.isEmpty()) {
            return new ValidationResponse(false, "API key not configured", 0);
        }

        try {
            String prompt = buildPrompt(word, categoryName, letter);
    ...
    private String buildPrompt(String word, String categoryName, Character letter) {
        String lang = LANGUAGE.equals("ar") ? "Arabic" : "English or French";
